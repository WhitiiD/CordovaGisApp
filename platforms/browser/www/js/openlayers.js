...
function addPointToLayer(point){	
	...
	//Erzeugt Koordinate aus dem Array coo und transformiert es
	var coord=ol.proj.fromLonLat(coo,'EPSG:3857');
	//Macht aus der Coordinate eine geometrie
	var geo=new ol.geom.Point(coord);
	// neues point-feature wird erzeugt.
	var point_feature = new ol.Feature({});	
	//Setzen des geometrietrypes
	...
	//Fügt dem Vector_layer der Map das Punkt-Feature hinzu
...
function sendWFST(){
	...
	featuresToInsert=vector_point;
	featuresToUpdate=null;
	featuresToDelete=null;
	
	
	
		var transactionRequest = (new ol.format.WFS()).writeTransaction(featuresToInsert, featuresToUpdate, featuresToDelete, {
	        gmlOptions: { srsName: "EPSG:3857" },
	        featureType: featureType,
	        featureNS: "http://geoserver.org/bw"
	    });
		
		
	    var transactionRequestXml = new XMLSerializer().serializeToString(transactionRequest);
	    transactionRequestXml = transactionRequestXml.replace("<Name>geometry</Name>", "<Name>geom</Name>");
	    // Name des Geometrie-Attributes muss manuell gesetzt werden, da der mit feature.setGeometryName("geom") gesetzte Bezeichner
	    // von writeTransaction() nicht berücksichtigt wird
	    $.ajax({
	        type: "POST",
	        url: "http://" + geoserverHost + ":" + geoserverPort + "/geoserver/wfs",
	        data: transactionRequestXml,
	        contentType: 'text/xml',
	        //success: function() { 
	        //   reloadLayer(featureType);
	        // }
	    });
}
